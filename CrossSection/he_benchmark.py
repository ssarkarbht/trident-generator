...
calculation at high energy (inclusive of low energy) energy where we can compare
 the results with some of the reported cross-section in the context of neutrino
 beam experiements
'''
...
#wox_data=np.loadtxt('/home/sourav/MC_Trident/wolfgang_ox.dat',delimiter=',')
...
ax1.scatter(inen,2*modxs,s=5,label='Sourav')
#plt.errorbar(inen,xsval/inen*1e-36,erval/inen*1e-36,fmt='none')
#plt.errorbar(inen,xsval/inen*1e-36,unval/inen*1e-36,fmt='none')
ax1.plot(new_cm,2*10**(box_func(np.log10(new_cm))))
ax1.scatter(box_data[:,0],box_data[:,1],s=5,label='Zhou, Beacom [arXiv:1910.10720v2]')
#ax1.scatter(wox_data[:,0],wox_data[:,1]/wox_data[:,0]*1e-39,label='Wolfgang generator')
ax1.plot(fox_data[:,0],fox_data[:,1]/fox_data[:,0]*1e-36,'r',label='Shao-Feng et. al. [arXiv:1702.02617v2]')
ax1.legend(fontsize=7)
ax1.set_xscale('log')
ax1.set_yscale('log')
ax1.set_title("Neutrin Trident Cross-section")
ax1.set_ylabel(r'$\sigma_{\nu A}/E_{\nu}$ [$cm^2GeV^{-1}$]')
ax1.set_xlabel('Neutrino Energy (GeV)')
ax1.set_ylim([1e-44,1e-39])
ax1.grid(which='both',alpha=0.3,ls=':')

ratio=box_data[:,1]/2./10**(box_func(np.log10(box_data[:,0])))
